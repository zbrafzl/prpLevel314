...
namespace Prp.Data
{
	public class EmailDAL : PrpDBConnect
	{
		...
		public Message EmailTemplateAddUpdate(EmailTemplate template)
		{
			...
			sqlCommand.Parameters.AddWithValue("@emailTemplateId", template.emailTemplateId);
			sqlCommand.Parameters.AddWithValue("@name", template.name);
		...
		public List<EmailTemplate> EmailTemplateSearch(EmailTemplate template)
		{
			...
			try
			{
				List<spEmailTemplateSearch_Result> list = this.db.spEmailTemplateSearch(new int?(template.inductionId), new int?(template.typeId), template.search).ToList<spEmailTemplateSearch_Result>();
				emailTemplates = MapEmail.ToEntityList(list);
			}
			catch (Exception exception)
			{
