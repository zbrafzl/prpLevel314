...
namespace Prp.Data
{
	public class ApplicantDAL : PrpDBConnect
	{
		...
		public Message ApplicantDistinctionAddUpdate(ApplicantDistinction obj)
		{
			...
			try
			{
				spApplicantDistinctionAddUpdate_Result spApplicantDistinctionAddUpdateResult = this.db.spApplicantDistinctionAddUpdate(new int?(obj.applicantDistinctionId), new int?(obj.inductionId), new int?(obj.phaseId), new int?(obj.applicantId), obj.subject, new int?(obj.year), obj.imageDistinction, new int?(obj.position), obj.university).FirstOrDefault<spApplicantDistinctionAddUpdate_Result>();
				message = MapApplicantDistinction.ToEntity(spApplicantDistinctionAddUpdateResult);
			}
			catch (Exception exception1)
			{
				Exception exception = exception1;
		...
		public Message ApplicantDistinctionDeleteSingle(int inductionId, int phaseId, int applicantDistinctionId)
		{
			...
			try
			{
				this.db.spApplicantDistinctionDeleteByApplicant(new int?(applicantDistinctionId), new int?(inductionId), new int?(phaseId));
				message.status = true;
			}
			catch (Exception exception1)
			{
		...
		public Message ApplicantSpecilityAddUpdate(ApplicantSpecility obj)
		{
			...
			try
			{
				spApplicantSpecilityAddUpdate_Result spApplicantSpecilityAddUpdateResult = this.db.spApplicantSpecilityAddUpdate(new int?(obj.applicantSpecilityId), new int?(obj.inductionId), new int?(obj.phaseId), new int?(obj.applicantId), new int?(obj.preferenceNo), new int?(obj.typeId), new int?(obj.specialityId), new int?(obj.hospitalId)).FirstOrDefault<spApplicantSpecilityAddUpdate_Result>();
				message = MapApplicant.ToEntity(spApplicantSpecilityAddUpdateResult);
			}
			catch (Exception exception1)
			{
				Exception exception = exception1;
		...
		public Message ApplicantSpecilityDeleteSingle(int inductionId, int phaseId, int applicantSpecilityId)
		{
			...
			try
			{
				this.db.spApplicantSpecilityDeleteByApplicant(new int?(applicantSpecilityId));
				message.status = true;
			}
			catch (Exception exception1)
			{
		...
		public Message ApplicantVoucherAddUpdate(ApplicantVoucher obj)
		{
			...
			try
			{
				this.db.spApplicantVoucherAddUpdate(new int?(obj.applicantId), new int?(obj.inductionId), new int?(obj.phaseId), new int?(obj.amount), obj.branchCode, obj.voucherImage, obj.ibn, obj.accountNo, obj.accountTitle, new DateTime?(obj.submittedDate));
				message.status = true;
				...
				try
				{
					try
					{
						...
						if (dataTable.Rows.Count > 0)
						{
							...
							applicantDistinction.university = str1;
						}
					}
					catch (Exception exception1)
					{
			...
			try
			{
				...
				if ((dataTable == null ? false : dataTable.Rows.Count > 0))
				{
					foreach (DataRow row in dataTable.Rows)
					{
						...
						applicationStatuses.Add(applicationStatu);
					}
				}
			}
			catch (Exception exception)
